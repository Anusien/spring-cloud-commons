...
=== Usage
You can configure a `WebClient` to do request hedging by creating a `HedgingClient` `@Bean` and use the `@Hedged`
qualifier. The `HedgingClient` allows you to specify how long to wait before executing the extra requests, a maximum
number of extra requests to create. Most users will want the `LatencyPercentileHedgingClient`, which executes new
...
public class MyConfiguration {
    ...
    @Hedged(hedgingClient = "hedgingClient", metricsReporters = {"hedgingReporterA", "hedgingReporterB"})
    @Bean
    WebClient.Builder webClientBuilder() {
        return WebClient.builder();
    }
