...
=== Reporting
Hedging also provides a `HedgerListener`. The extra requests generated by hedging could produce false positives
in monitoring. Instances of `HedgerListener` wired into the hedging `ExchangeFilter` will receive only the
...
[source,java,indent=0]
----
import org.springframework.cloud.client.hedger.HedgerListener;@Configuration
public class MyConfiguration {
    @Bean
    HedgingPolicy hedgingPolicy() {
    	return new LatencyPercentileHedgingPolicy(
    ...
    @Bean
    HedgerListener hedgingReporterA() {
		return new HedgerListener() {
			@Override
    ...
    @Bean
    HedgerListener hedgingReporterB() {
		return new HedgerListener() {
			@Override
			public void record(ClientRequest request, ClientResponse response, long elapsedMillis, Integer hedgeNumber) {
				// TODO Do your business...
