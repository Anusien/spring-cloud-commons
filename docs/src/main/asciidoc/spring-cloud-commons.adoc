...
=== Usage
You can configure a `WebClient` to do request hedging by creating a `HedgingPolicy` `@Bean` and use the `@Hedged`
qualifier. The `HedgingPolicy` allows you to specify how long to wait before executing the extra requests, a maximum
number of extra requests to create. Most users will want the `LatencyPercentileHedgingPolicy`, which executes new
...
=== Reporting
Hedging also provides a `HedgingMetricsReporter`. The extra requests generated by hedging could produce false positives
in monitoring. Instances of `HedgingMetricsReporter` wired into the hedging `ExchangeFilter` will receive only the
single request attempt that was used by the caller, ignoring any other attempts that were made and discarded.  These
reporters are wired in to the `@Hedged` qualifier, as in this example:

====
[source,java,indent=0]
----
import org.springframework.cloud.client.hedger.HedgingMetricsReporter;@Configuration
public class MyConfiguration {
    @Bean
    ...
    @Bean
    HedgingMetricsReporter hedgingReporterA() {
		return new HedgingMetricsReporter() {
			@Override
			public void record(ClientRequest request, ClientResponse response, long elapsedMillis, Integer hedgeNumber) {
    ...
    @Bean
    HedgingMetricsReporter hedgingReporterB() {
		return new HedgingMetricsReporter() {
			@Override
			public void record(ClientRequest request, ClientResponse response, long elapsedMillis, Integer hedgeNumber) {
				// TODO Do your business...
    ...
    @Hedged(hedgingPolicy = "hedgingPolicy", metricsReporters = {"hedgingReporterA", "hedgingReporterB"})
    @Bean
    WebClient.Builder webClientBuilder() {
        return WebClient.builder();
    }
